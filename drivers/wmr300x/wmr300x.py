...
Sensor data transmission frequencies:
  wind: 2.5 seconds (always 48 per minute, but not always precise interval -
        every 2nd reading is always a duplicate of the first.
    TH: 10 to 12 seconds
...
11   A8 
12   00      average speed (prev 10 sec block)  2.9 m/s
13   1D 
...
PyUSB_major_version = 0
pyusb_version_string = usb.__version__
        # the 1.0.0.beta2 release seems to be missing some functionality,
        # but was shipped with Ubuntu 16.  Only use PyUSB v1 API for released versions
if pyusb_version_string.startswith("0.") or pyusb_version_string.startswith("1.0.0b"):
    PyUSB_major_version = 0
else:
    PyUSB_major_version = 1
    import usb.core
    import usb.util
...
def logexception( msg, e):
    ...
    log.error( "%s exception: %s (#%s), backend: %s" % (msg, e.strerror, errnostr, str(e.backend_error_code)) )
...
class WMR300Driver(weewx.drivers.AbstractDevice):
    ...
    # this flag determines whether to used the console-generated wind average or to use
    # weewx to do the averaging.
    # The console averaging is very crude and does not align with archive interval, so best avoid.
    IGNORE_WIND_AVGE = 1
    ...
    def __init__(self, **stn_dict):
        ...
        self.ignore_windavge = int(stn_dict.get('ignore_wind_avge', self.IGNORE_WIND_AVGE) )
        if self.ignore_windavge:
            self.sensor_map['windSpeed'] = 'wind_current_spd'
            self.sensor_map['windDir'] = 'wind_current_dir'
        ...
        global DEBUG_WIND
        DEBUG_WIND = int(stn_dict.get('debug_wind', DEBUG_WIND))
        if PyUSB_major_version == 1:
        ...
        self.heartbeat = 37 # how often to send a6 messages, in seconds
        ...
        while cnt < max_tries:
            ...
            try:
                ...
                if e.backend_error_code == self.station.backend_timeout_code :
                    logexception( "init_comm ", e)
                elif not ('No error' in errmsg):
                    logexception( "USB failure in init_comm ", e)
                    raise weewx.WeeWxIOError(e)
            except (WrongLength, BadChecksum, InitiationError) as e:
                log.error( "initComms: " + repr(e))
            time.sleep(0.1)
    ...
    def init_history(self, clear_logger=False, max_tries=5, max_read_tries=10):
        ...
        while cnt < max_tries:
            ...
            try:
                ...
                errmsg = repr(e)
                if e.backend_error_code == self.station.backend_timeout_code :
                    logexception( "init_history ", e)
                elif not ('No error' in errmsg):
                    logexception( "USB failure in initiateHist ", e)
                    raise weewx.WeeWxIOError(e)
            except (WrongLength, BadChecksum, InitiationError) as e:
                log.error( "HistRead: " + repr(e) )
            time.sleep(0.1)
    ...
    def get_history(self, since_ts, clear_logger=False):
        ...
        while True:
            try:
                ...
                errmsg = repr(e)
                if e.backend_error_code == self.station.backend_timeout_code :
                    logexception( "get_history (from "+state+")", e)
                elif not ('No error' in errmsg):
                    logexception( "USB failure in get_history (from "+state+")", e)
                    raise weewx.WeeWxIOError(e)
                else:
                    logexception( "get_history no error?", e)
    ...
    def genLoopPackets(self):
        ...
        while True:
            try:
                ...
                buf = self.station.read()
                read_loop_time = time.time()
                read_return_delta = read_loop_time - read_enter_time 
                if buf:
                    if buf[0] in [0xd3, 0xd4, 0xd5, 0xd6, 0xdb, 0xdc]:
                        ...
                        if buf[0] in [0xd3, 0xd4, 0xd5, 0xd6]:
                            ...
                            if DEBUG_WIND and pkt['packet_type'] == 0xd4 :
                                log.info('WIND:  %.1f %s' % (read_loop_time, packet))
                            yield packet
                            if DEBUG_TIMING:
                ...
                errmsg = repr(e)
                if e.backend_error_code == self.station.backend_timeout_code :
                    logexception( "genLoopPackets (from "+state+")", e)
                elif not ('No error' in errmsg):
...
class Station(object):
    ...
    def open(self):
        ...
        if self.backend == Station.BACKEND_LIBUSB0:
            ...
            if kernelactive :
                log.info( "Kernel driver is active, detaching interface %d" % self.interface )
                dev.detach_kernel_driver( self.interface )
    ...
    def is_timeout(self, e):
        if hasattr(e, 'backend_error_code') and e.backend_error_code == self.backend_timeout_code:
            return True
        errmsg = repr(e)
        for msg in USB_TIMEOUT_MESSAGES:
            if msg in errmsg:
    ...
    def read(self, count=True, timeout=None, ignore_non_errors=True, ignore_timeouts=True):
        try:
            ...
            if ignore_timeouts and self.is_timeout(e):
                return []
    ...
    def _find_dev(vendor_id, product_id):
        """Find the first device with vendor and product ID on the USB.
            This is only used for Pyusb version 0"""
    ...
    def _decode_d2(buf):
        ...
        # what is reported as "gust" in loop data is just the most recent value.
        # The time taken to make a single reading is 2.5 sec, but results are
        # reported every 1.25 sec, so "gust" is only ever the current wind reading
        current_wind_spd = Station._extract_value(buf[72:74], 0.1) # m/s
        current_wind_dir = Station._extract_value(buf[76:78], 1.0) # degree
        pkt['wind_gust'] = current_wind_spd
        pkt['wind_gust_dir'] = current_wind_dir
        avge_wind_speed = Station._extract_value(buf[74:76], 0.1) # m/s
        avge_wind_dir = Station._extract_value(buf[78:80], 1.0) # degree
        pkt['wind_avg'] = avge_wind_speed
        pkt['wind_dir'] = avge_wind_dir
        pkt['wind_current_spd'] = avge_wind_speed
        pkt['wind_current_dir'] = avge_wind_dir
    ...
    def _decode_d4(buf):
        ...
        pkt['ts'] = Station._extract_ts(buf[2:7])
        pkt['channel'] = buf[7]
        current_wind_spd = Station._extract_value(buf[8:10], 0.1) # m/s
        pkt['wind_gust'] = current_wind_spd 
        current_wind_dir =Station._extract_value(buf[10:12], 1.0) # degree
        pkt['wind_current_spd'] = current_wind_spd
        pkt['wind_gust_dir'] = current_wind_dir
        pkt['wind_current_dir'] = current_wind_dir
...
if __name__ == '__main__':
    ...
    if PyUSB_major_version == 1:
        log.debug( "Station is %s, prod: %s, by: %s" % (stn.hardware_name, stn.station.dev.product, stn.station.dev.manufacturer) )
